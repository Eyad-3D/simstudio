...
in turn (113 faults) and Data Checks run on the result. A fault must never
...
redo that measurement when the examples change.
"""
import copy
...
HARMLESS = {
    ...
    "hybrid-car": {"el-monitor", "el-ground", "c-14", "c-20",
...
STOPS_THE_CAR = {
    "bev-car": {"el-vehicle", "el-driver", "el-node-fl", "el-hvbus", "el-battery", "el-motor",
                "el-final-drive", "el-diff", "el-node-fr", "el-task", "c-1", "c-4", "c-5",
                "c-6", "c-7", "c-8", "db-1", "db-2"},
    "hybrid-car": {"el-vehicle", "el-driver", "el-task", "el-engine", "el-node", "el-gearbox",
                   "el-fd", "el-diff", "el-battery", "el-hvbus", "el-motor", "el-node-l",
                   "el-node-r", "c-1", "c-2", "c-3", "c-4", "c-5", "c-6", "c-7", "c-8", "c-13",
                   "c-15", "db-1", "db-4", "db-5"},
}
# ... of which these still leave a complete, commanded drive path: without its
# E-Motor the P2 hybrid's control script never starts the engine. Data Checks
# warn (the battery then supplies nothing) but cannot know the strategy.
...
def test_corpus_is_complete(outcome):
    assert sum(len(v) for v in outcome.values()) == 113
    for name in EXAMPLES:
...
def test_check_coverage_is_at_least_90_percent(outcome):
    ...
    assert len(consequential) == 91
    assert len(flagged) / len(consequential) >= 0.9  # 89 of 91 today
