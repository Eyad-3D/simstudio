...
| Run status | a car that cannot follow the cycle, does not move, or runs out of energy is never reported as a success; untrustworthy figures carry a "not valid" flag; a run a stop cut short is *cancelled*, one stopped as it ended is not; a 0-100 km/h performance test succeeds with the time of a full-throttle run, and a top-speed test reports the car's highest speed | `test_verdict.py`, `test_examples_plausible.py`, `test_api.py` |
