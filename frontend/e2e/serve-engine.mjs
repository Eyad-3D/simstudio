 ...
 * folder seeded with the bundled examples, so tests can save freely without
...
const projects = mkdtempSync(join(tmpdir(), "simstudio-e2e-"));
for (const name of readdirSync(join(backend, "projects"))) {
  if (name.endsWith(".json")) copyFileSync(join(backend, "projects", name), join(projects, name));
}
