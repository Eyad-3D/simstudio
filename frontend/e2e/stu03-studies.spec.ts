...
test("STU-03: two sweeps give two saved studies that survive a reload and a save", async ({ page }) => {
  ...
  expect((await tableOf(page, 0)).map((row) => row[0])).toEqual(["900", "1800", "2700"]);
  ...
  expect(saved.studies[0].factors[0]).toMatchObject({ elementLabel: "Vehicle", paramLabel: "Vehicle Mass", values: [900, 2700] });
